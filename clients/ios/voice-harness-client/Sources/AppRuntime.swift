...
final class AppRuntime: ObservableObject {
    ...
    /// me_email). On iOS the platform-uuid and me-email providers compile to
    /// no-ops (they always yield nil), so `icloud` is the only key that can
    /// resolve here — once the app is provisioned with the iCloud
    /// entitlement. Unsafe-annotated like `AppSettings.defaults` — replaced
